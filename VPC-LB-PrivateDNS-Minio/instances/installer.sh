...
checkin() {
curl -X POST --data-urlencode "payload={\"channel\": \"#ibmcloud\", \"username\": \"webhookbot\", \"text\": \"System `hostname -s` has come online system with a primary ip of: `hostname -I`\"}, \"icon_emoji\": \":ghost:\"}" "${SLACK_URL}"
} >> "$installerlog" 2>&1
